...
fn test_video_output() -> Result<()> {
    ...
    let mut context = ffmpeg::format::output(&"/home/yalter/test.mkv")
    ...
    let mut encoder = {
        ...
        stream.set_rate((60, 1));

        println!("stream tb={:?} fps={:?}", stream.time_base(), stream.rate());
...

    {
        ...
        for i in 0..60 {
            ...
            if encoder.encode(&frame, &mut packet).chain_err(|| "could not encode the frame")? {
                println!("Encoded packet pts={:?} duration={}", packet.pts(), packet.duration());
        ...
        while encoder.flush(&mut packet).chain_err(|| "could not get the packet")? {
            println!("Flushed packet pts={:?} duration={}", packet.pts(), packet.duration());
